...
| スキル | 何をするか | トークン目安 |
|---|---|---|
| `/repo-audit-min` | 機械判定 43 項目 (ファイルの有無・git/CI の状態・`gh api` によるリポ設定) だけを走らせ、**逸脱した項目を 1 行ずつ**に圧縮して出す。LLM 判定・findings 保存・修正提案はしない | **1K 未満** (スキル本文 + 出力の合計) |
| `/repo-audit` | 上に加えて LLM 判定 6 項目 (ADR の網羅性、CLAUDE.md の質、`.claude/` 設定、作業ログの外部化など) をサブエージェントで判定し、結果を findings に保存して修正フローへ渡す | メインコンテキスト **+10K 前後**、サブエージェント込みの累積 **300〜600K** |
| `/repo-audit-fix` | findings の未決項目を承認を取りながら修正する (リポ内ファイルは 1 PR にまとめ、GitHub 設定は定義ファイル経由、破壊的操作は提示のみ) | 修正件数しだい。`/repo-audit` と同等かそれ以上 |

`/repo-audit` の累積が大きいのは、LLM 判定 6 項目がそれぞれ ADR 全文・CLAUDE.md・`.claude/` 配下を読むため (このリポでは判定材料だけで約 12K トークン、うち ADR が 9K)。**ADR の本数が支配的**なのでリポの規模でかなり動く。ただし findings は前回の判定を id 単位で引き継ぐため、**2 回目以降は status が変わった項目だけの再判定で済み大幅に安い**。
