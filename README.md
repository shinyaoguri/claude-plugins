...
| [repo-standards](plugins/repo-standards) | 自分の全リポジトリと各マシン | 個人の開発運用標準 ([setup](https://github.com/shinyaoguri/setup) の repo-standards.json が正本) — リポジトリ構成・GitHub 設定・マシン環境の監査と雛形生成、作業の記録 (Issue・PR へのスクショ添付・気付きの起票) の作法、`/repo-audit` `/repo-audit-min` `/repo-audit-fix` `/env-doctor` `/repo-bootstrap` `/gyazo-capture` `/report-issue` (skill 7)、push した PR の CI が赤いまま終わらせない見届け (PostToolUse + Stop hook 2、[ADR 0016](docs/decisions/0016-agent-behavior-hooks-in-plugin.md)) |
...
| スキル | 何をするか | コスト目安 |
|---|---|---|
| `/repo-audit-min` | 機械判定 43 項目を圧縮して報告し、LLM 判定 6 項目は材料をスクリプトで集めてから **Haiku 1 本**に一括で任せる。求められれば判定を**暫定値**として findings に残し修正フローへ渡す (既定は保存せず、修正の提案もしない) | メイン **1K 弱** + 判定係 **1 本**。概算 **$0.05 前後** |
...
**この標準は全リポ共通の既定であって、個別のリポで最適とは限らない** ([ADR 0013](docs/decisions/0013-standard-vs-repo-intent.md))。`/repo-audit` は指摘ごとに「その fix を当てるとリポ自身が明示している設計意図 (CLAUDE.md・ADR・README・as-code の設定) と衝突しないか」を判定し、衝突する項目は修正フローで**提示のみ**に落とす。この判定は機械判定 43 項目にも効く — LLM が文脈を持ち込める接点はここしかないため。衝突と言えるのは**リポにコミットされた記述に根拠があるときだけ**で、根拠なく逸脱を認めると標準を黙らせる装置になる。なお `intent` は判定そのものを上書きしない: 標準から外れている事実は残したまま、扱いだけを変える。

**判定は出自ごと記録される** ([ADR 0015](docs/decisions/0015-verdict-provenance.md))。`/repo-audit-min` の判定は `verdict_source: min` の暫定値として保存され、**陳腐化していない `/repo-audit` の判定を上書きしない**。安い監査で見つけた必須違反はそのまま `/repo-audit-fix` へ運べる一方、暫定値は次に `/repo-audit` を回せば必ず判定し直される。機械判定を LLM 判定が覆した項目 (偽陽性) も、機械判定の事実を残したまま「なぜ直さないのか」を findings に持てる。
