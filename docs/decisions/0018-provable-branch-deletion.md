...
- **状態**: 一部廃止 (2026-09-22。証明 (b)「`[gone]` かつマージ済み PR の head と tip が一致」と、その環境変数 `RS_BRANCH_SWEEP_GONE` / `RS_BRANCH_SWEEP_GONE_MAX` → [0030](0030-simplify-the-audit.md) 決定 4。証明 (a)「既定ブランチの祖先」は現行)。採用は 2026-08-14
