...
# このフックはプラグインが供給するので全プロジェクトで動く。対象外の状況では
# 何も書かずに即 exit 0 する (経緯: shinyaoguri/claude-plugins#86 / ADR 0016)。
#
# stdin: PostToolUse の JSON (.tool_input.command / .cwd / .session_id)
