...
- `repo-uninitialized` が載っている → コミットが 1 件も無いリポ。修正フローでなく **repo-bootstrap スキル**へ渡す (監査が打ち切られているので findings にはこの 1 件しかなく、直す対象がそもそも並んでいない)
...
- `反証待ち N 件` → repo-audit の手順 3 (反証) を先に済ませることを勧める。反証にかかるのは required のうち ok / skip と判定された項目で、覆れば**直す対象が増える**。通さずに進むと、必須の違反を見落としたまま修正を終えることになる。ユーザーが承知のうえで進めるなら続行してよい
...
3. 群ごとに番号付きで並べた**一覧を 1 回提示し、まとめて承認を取る**。生成的な項目は「何を書き起こすか」を 1 行で添える (全文は出さない — 読むのは PR の diff で足りる)。

   **リポから材料を読み取れない項目は、この 1 回の提示に質問を同梱する。** 生成的 fix はリポの実体を材料にするので、材料が無いまま当てると見出しだけの雛形が残り、次の監査でも実質未整備のまま通ってしまう。承認とは別の往復を作らず、一覧の末尾に最小限だけ並べる:

   | 項目 | リポから読めなければ聞くこと |
   |---|---|
   | `readme-exists` | このリポが何をするものか (1 行) |
   | `claude-md-exists` | 検証コマンド (ビルド・テスト・lint) と、非自明な注意 |
   | `ci-workflow-exists` / `tests-run-in-ci` | CI で回す検証コマンド |
   | `dependabot-config` | エコシステム (`_meta` の kind が generic のときだけ) |
   | `license-exists` | 既定は MIT。変えるなら何にするか |

   **先に自分で読んで埋め、埋まらなかったものだけ聞く** — marker ファイル (package.json・Package.swift・pyproject.toml)、既存の CI 設定、Makefile や npm scripts、git log のコミット傾向から分かることは質問にしない。答えが得られなかった項目は生成せず `deferred` にする (推測で README に事実でない説明を書かない)。

   承認されなかった項目も findings に記録してから次へ進む (記録しないと次のセッションで同じ確認を繰り返す):
...
4. 承認された項目を適用する。`chore/repo-standards` ブランチを切り、群 1・2 をまとめて **1 PR** にする (標準適合が 1 関心事なので、項目ごとに PR は作らない)。
   ...
   **ディレクトリだけ作って終わらせない。** git は空ディレクトリを追跡しないので、`docs/decisions/` やテストディレクトリを作るだけではコミットに乗らず、適用したつもりで再監査の `applied_unresolved` に残る。中身を伴わせる — ADR なら「ADR で設計判断を残すと決めた」こと自体を 0000 番として書き、テストディレクトリなら kind の慣習に沿った実行可能なテストを 1 本置く。
