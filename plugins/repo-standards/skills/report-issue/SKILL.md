...
6. **見た目・動き・GUI が絡む事象なら視覚証跡を貼る**。修正 PR に証跡を求めるリポジトリでも、**「壊れている絵」を撮れるのは起票の時点だけ**で、直った絵しか残らなくなる。動きが分からないと正誤を判定できないものは静止画に加えてアニメーション WebP も併載する (判断基準と Gyazo への上げ方は gyazo-capture スキルが正本。リポジトリに画像をコミットしない)。撮れないときは撮れない理由を 1 行書いて起票する (**証跡が無いことを起票しない理由にしない**)。
