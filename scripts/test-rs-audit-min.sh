...
echo
echo "--save は持たない (報告専用):"

# 判定を findings へ保存して修正フローへ渡す経路は畳んだ (ADR 0030)。直すなら本監査を通す
( cd "$outside" && REPO_STANDARDS_JSON="$manifest" bash "$target" --no-github --save >/dev/null 2>&1 ); c=$?
[ "$c" -eq 2 ] && ok "--save は未知の引数として exit 2" || fail "--save → exit=$c"
