...
echo
echo "判定の出自 (--source) は持たない"

# repo-audit-min から判定を保存する経路は畳んだ (ADR 0030)。黙って受け付けると、古い手順で
# 書き込んだ判定が出自なしの本監査の判定として残る
d=$(newrepo)
got=$( cd "$d" && line a manual | bash "$target" save >/dev/null
       bash "$target" set --verdict ok --source min --evidence "$EV" a >/dev/null 2>&1; echo "$?" )
check "2" "$got" "--source は未知の引数として拒否する"

d=$(newrepo)
got=$( cd "$d" && line a manual | bash "$target" save >/dev/null
       bash "$target" set --verdict ok --evidence "$EV" a | jq -r 'has("verdict_source")' )
check "false" "$got" "行に verdict_source を書かない"
