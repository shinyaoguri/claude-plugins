...
- **人間の承認はプラン 1 点へ集約する** (ADR [0017](docs/decisions/0017-approval-at-the-plan.md))。可逆な操作に確認を挟まない代わりに、合意なしの実装拡大は `plan-gate` フックが deny で止める。分類器の設定 (`autoMode.*`) は仕様上プラグインから供給できないので、プラグインは **`env-doctor` で診断するだけ**にとどめ、実体は setup リポの `claude/settings.json` に置く
